...
#input: rank_a[i] (and similarly rank_b) is a vector containing the rank of item i where 1 indicates the best, 2 the second best, and so on. if item i is unranked, set rank_a[i] = -1
# def kendall_tau_distance(rank_a, rank_b):
#     KT_distance = 0
#
#     if(rank_a.shape[0] < rank_b.shape[0]):
#         smaller_rank = np.copy(size_a)
#         larger_rank = np.copy(size_b)
#     else:
#         smaller_rank = np.copy(size_b)
#         larger_rank = np.copy(size_a)
#
#     for i in range(larger_rank.size[0]):
#         for j in range(smaller_rank.size[0]):
#             #both items have been ranked
#             if(rank_a[i] !=-1 and rank_b[i]!=-1):
#
#             #if only one item has been ranked
#             elif(rank_a[i]!=-1 and rank_b[i]==-1):
#
# def kendall_tau_distance(rank_a, rank_b):
#     KT_distance = 0
